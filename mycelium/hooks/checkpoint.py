...
def _build_reason(window_start: int, window_end: int, since: int) -> str:
    return (
        ...
        "ROUTING (two KBs) — recall from liam-mycelium (all history lives there); write rain "
        "work-related items to BOTH rain-mycelium AND liam-mycelium (same content, two calls); "
        "personal / non-rain → liam-mycelium only. Never link across the two KBs. If you are "
        "UNSURE whether something is personal or rain-related, ASK rather than assume — a wrong "
        "guess leaks personal content into the team KB, or buries team knowledge in the private one.\n\n"
        ...
        "passing `source` = the raw origin you copied it from (command, file path, paste, "
        "transcript turns). VERBATIM means COPIED, not composed: if you'd be writing prose "
        "from your own understanding (SYMPTOM / ROOT CAUSE / FIX / DECISION headers YOU "
        "wrote), that belongs in Pass 4 as a write_note, NOT a drawer. Test per item: could "
        "you regenerate it byte-for-byte by re-running a command or re-reading a file? "
        ...
        "PASS 4 — SYNTHESIS + LINK (judgment allowed here, and only here)\n"
        ...
        "before deciding.\n"
        "  3. LINK — MANDATORY, NOT OPTIONAL. Every drawer filed in Pass 2 and "
        "every note written/upserted in this pass MUST be connected into the graph "
        "with mycelium-add-link: link each new entity to the note(s) it elaborates "
        "(part_of), to the work it extends / depends_on / causes, and to clearly "
        "related existing entities. Disconnected drawers/notes are a capture FAILURE "
        "— the typed graph is the only thing that makes this findable again. Use "
        "typed relations (part_of, extends, depends_on, causes, implements, "
        "contradicts, related_to). Links stay WITHIN one KB — NEVER link across the "
        "two KBs. End with a reconciliation line: \"linked: X edges across N "
        "entities; 0 orphans\" — if an entity is genuinely standalone, say so "
        "explicitly rather than leaving it silently unlinked.\n"
        "You MAY skip writing new notes (steps 1–2) if nothing has solidified — but "
        "you may NEVER skip Pass 2, and you may NEVER skip the Pass 4 LINK step (3) "
        "whenever Pass 2 filed anything.\n\n"
